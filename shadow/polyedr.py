...
def is_good_point(p, koef_dist):
    y_original = p.y / koef_dist
    return y_original > 1 or y_original < -3
...
class Facet:
    ...
    def area(self):
        ...
        for i in range(1, len(v) - 1):
            area += triangle_area(v[0], v[i], v[i + 1])

        return area

    def has_two_good_vertexes(self, koef_dist):
        count = sum(1 for v in self.vertexes if is_good_point(v, koef_dist))
        return count >= 2
...
class Polyedr:
    ...
    def good_facets_area(self):
        total = 0.0

        for facet in self.facets:
            if facet.has_two_good_vertexes(self.koef_dist):
                total += facet.area()

        return total / self.koef_area
